...
function DiceField(props) {
  ...
  const [thirdSkull, setThirdSkull] = useState(false);
  ...
  useEffect(() => {
      console.log("PLAYER ID: " + props.playerIDForGame)
  ...
  useSubscription("/topic/getmarkedcells", (message) => {
    ...
    // console.log("player to mark cells: " + JSON.parse(message.body).playerToMarkCells);
  ...
  useSubscription("/topic/getthreeskulls", (message) => {
    console.log("skull recieved");
    if (JSON.parse(message.body).playerToMarkCells === 1) {
      props.setPlayerToMarkCells(2);
    } else {
      props.setPlayerToMarkCells(1);
  ...
  const sendThreeSkulls = (threeSkulls) => {
    if (stompClient) {
      stompClient.publish({
        destination: "/app/threeskulls",
        body: JSON.stringify(threeSkulls),
      });
    } else {
      //Handle error
    }
  ...
  const getRolledDicesColorAndSendThemToServer = (diceColor, number) => {
    if (props.playerIDForGame === props.startingPlayer) {
      ...
      if (
        props.diceRollResults.diceRolls.length === props.numberOfRolledDices
      ) {
  ...
  const handleClickOnDice = (diceColor) => {
    ...
    if (rollingIsOver && props.playerToMarkCells === props.playerIDForGame) {
      ...
      props.playerToMarkCells === props.playerIDForGame
    ) {
  ...
  const selectDiceForReroll = (diceColor) => {
    if (props.rerollCounter !== "third3" && diceColor !== 5) {
  ...
  const cancelDiceForRerollForStartingPlayer = (diceColor) => {
    if (
      props.startingPlayer === props.playerIDForGame &&
      props.ownerOfDiceField === props.playerIDForGame
    ) {
      cancelDiceForReroll(diceColor);
    }
  };
  ...
  const checkForThreeSkulls = (group) => {
    if (group.length === 3) {
      setThirdSkull(true);
    }
  };
  ...
  const endRollingPhase = (value) => {
    ...
    setRollingIsOver(true);
    console.log("STOP");
    console.log(thirdSkull);
    if (thirdSkull) {
      let threeSkulls = {
        threeSkulls: true,
        playerToMarkCells: props.playerToMarkCells,
      };
      sendThreeSkulls(threeSkulls);
    }
  };
  ...
  const getNumberOfMarkedCells = (diceColor, playerToMarkCells) => {
    if (diceColor === 1 && playerToMarkCells === 1) {
      return props.markedPurpleCellsCounterPlayerOne;
    } else if (diceColor === 1 && playerToMarkCells === 2) {
      return props.markedPurpleCellsCounterPlayerTwo;
    } else if (diceColor === 2 && playerToMarkCells === 1) {
      return props.markedBlackCellsCounterPlayerOne;
    } else if (diceColor === 2 && playerToMarkCells === 2) {
      return props.markedBlackCellsCounterPlayerTwo;
    } else if (diceColor === 3 && playerToMarkCells === 1) {
      return props.markedOrangeCellsCounterPlayerOne;
    } else if (diceColor === 3 && playerToMarkCells === 2) {
      return props.markedOrangeCellsCounterPlayerTwo;
    } else if (diceColor === 6 && playerToMarkCells === 1) {
      return props.markedTurquoiseCellsCounterPlayerOne;
    } else if (diceColor === 6 && playerToMarkCells === 2) {
      return props.markedTurquoiseCellsCounterPlayerTwo;
    }
  };

  const selectDiceColorToMarkCells = (
    selectedDiceColor,
    playerToMarkCells,
    ownerOfDiceField
  ) => {
  ...
  const startNewTurn = () => {
    ...
    } else if (props.ownerOfDiceField !== props.startingPlayer) {
      props.setDicesVisible("visible");
      props.setRollButtonVisible("visible");
      props.setRerollCounterVisible("visible");
      setRollingIsOver(false);
    }
    if (props.playerIDForGame === props.startingPlayer && props.ownerOfDiceField !== props.playerIDForGame){
      ...
      <Row>
        ...
        <Col>
          <RerollSelectionField
            ...
            cancelDiceForRerollForStartingPlayer={
              cancelDiceForRerollForStartingPlayer
            }
